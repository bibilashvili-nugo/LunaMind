...
// Input component
const InputTeacherInfo: React.FC<{
  value: string;
...
const TeacherInfo: React.FC<StudentInfoProps> = ({
  user,
  fullName,
  setFullName,
}) => {
  ...
  const updateBackend = useCallback(
    async (field: string, value: string) => {
      ...
      try {
        await fetch("/api/teachers/updateProfile", {
          method: "PATCH",
  ...
  return (
    <div className="mt-4 flex flex-col gap-4">
      <div className="flex flex-col gap-3 md:flex-row md:gap-4">
        <InputTeacherInfo value={fullName} onChange={setFullName} />
        <InputTeacherInfo value={age} onChange={setAge} />
      </div>
      <div className="flex flex-col gap-3 md:flex-row md:gap-4">
        <InputTeacherInfo value={phone} onChange={setPhone} />
        <InputTeacherInfo value={email} onChange={setEmail} />
      </div>
      <div className="flex flex-col gap-3 md:flex-row md:gap-4">
        <InputTeacherInfo value={country} onChange={setCountry} />
        <InputTeacherInfo value={city} onChange={setCity} />
        <InputTeacherInfo value={address} onChange={setAddress} />
      </div>
    </div>
  );
};
